...
async function processLineByLine() {
    const fileStream = fs.createReadStream(".env");const rl = readline.createInterface({
        input: fileStream,
        crlfDelay: Infinity,
    });
...
function writeToEnv(key = "", value = "") {
    ...
    const results = await processLineByLine();writeToEnv();for (const result of results) {
