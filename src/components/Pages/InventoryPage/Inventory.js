...
const Inventory = ({ infoAccount }) => {
    ...
    // Create fake assets for different sections based on available images
    ...
    if (itemsFiltered.length !== filterBySection(allItemsForSection, section).length) {
        setItemsFiltered(filterBySection(allItemsForSection, section));
    }
            ...
            <Stack direction="row" spacing={2} mb={4}>
                {['INGREDIENT', 'TOOL', 'FLASK', 'RECIPE', 'CREATION'].map(type => (
                    <Button
                        ...
                        onClick={() => {
                            setSection(type);
                            setItemsFiltered(filterBySection(allItemsForSection, type));
                        }}
                    >
